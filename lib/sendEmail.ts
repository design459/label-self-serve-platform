...
// (https://resend.com). Requires RESEND_API_KEY in the environment; throws
// a clear error instead of silently no-opping if it's missing, so a
// caller's catch block can surface "email isn't configured yet" rather
// than pretending to succeed. RESEND_FROM_EMAIL is optional — defaults to
// Resend's shared sandbox sender, which works immediately with no domain
// verification since these emails only ever go to one internal staff
// address, never to customers.
